...
// Helper function to call Gemini with exponential retries and fallback models
async function callGeminiWithRetryAndFallback(
  ...
  // Try newer and different Flash options to guarantee high availability
  ...
  for (const model of models) {
    for (let attempt = 1; attempt <= maxRetriesPerModel; attempt++) {
      try {
        console.log(`[AI SERVICE] Intentando llamar a Gemini con modelo: ${model} (Intento ${attempt}/${maxRetriesPerModel})`);
        ...
        if (response && response.text) {
          console.log(`[AI SERVICE] ¡Llamada a Gemini exitosa usando el modelo ${model}!`);
          return response;
        }
        ...
        console.warn(`[AI SERVICE] Error llamando a modelo ${model} (intento ${attempt}):`, errStr);

        // Detect high-demand/transient/limit errors
        const isTransient = 
          errStr.includes("503") || 
          errStr.includes("429") || 
          errStr.includes("demand") || 
          errStr.includes("demanda") || 
          errStr.includes("NO DISPONIBLE") || 
          errStr.includes("overloaded") ||
          errStr.includes("unavailable") ||
          errStr.includes("Service Unavailable") ||
          errStr.includes("temporales");

        if (!isTransient) {
          console.warn(`[AI SERVICE] Error no transitorio o de configuración. Cambiando de modelo de inmediato.`);
          break; // Try next model immediately
        }

        if (attempt < maxRetriesPerModel) {
          const delay = baseDelayMs * Math.pow(2, attempt - 1);
          console.log(`[AI SERVICE] Demanda alta de modelo detectada. Esperando ${delay}ms antes de reintentar...`);
  ...
  // Format a clean, human-friendly error if everything failed
  let cleanErrorMessage = "El modelo de Inteligencia Artificial de Gemini está experimentando picos de demanda críticos en tu región. Por favor, vuelve a intentarlo en unos instantes.";
...
app.post("/api/searches", async (req, res) => {
  ...
  try {
    ...
    const response = await callGeminiWithRetryAndFallback(ai, userPrompt, systemInstruction, responseSchema);
